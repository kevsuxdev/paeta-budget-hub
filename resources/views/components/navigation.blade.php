<aside class="min-w-72 bg-primary text-background h-screen px-6 py-8 flex flex-col">
    <div class="flex items-center gap-2 mb-5">
        ...
        <h1 class="text-xl font-bold ">Paete Budget Hub</h1>
    </div>
    <nav class="space-y-2">
        @auth
    ...
    <div class="mt-auto">
        <div class="mb-4 flex items-center gap-2">
            <div class="w-10 h-10 bg-accent rounded-full flex items-center justify-center text-white text-lg font-bold">
                {{ strtoupper(substr(auth()->user()->full_name, 0, 1)) }}
            </div>
            <div>
                <h3 class="text-base font-semibold">{{ auth()->user()->full_name }}</h3>
                <p class="text-sm text-gray-300">{{ ucfirst(auth()->user()->role) }}</p>
            </div>
        </div>
        <form method="POST" action="{{ route('auth.logout') }}">
            @csrf
            <button type="submit" class="w-full text-left px-4 py-2 text-white text-sm bg-orange-brown cursor-pointer rounded-md transition-colors flex items-center">
