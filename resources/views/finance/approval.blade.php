...
<div class="p-6">
    ...
    <div class="bg-orange-brown rounded-lg shadow-lg border border-black overflow-hidden">
                    ...
                    @forelse($budgets as $budget)
                        <tr class="hover:bg-primary/20 transition-colors">
                           ...
                           <td class="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                                ...
                                <button
                                    type="button"
                                    class="btn-approve-budget px-3 py-1 text-sm text-white border border-black bg-secondary rounded hover:bg-secondary/50 font-medium"
                                    data-budget-id="{{ $budget->id }}"
                                    data-budget-title="{{ $budget->title }}"
                                >
                                    Approve
                                ...
                                <form action="{{ route('finance.budget.finalReject', $budget->id) }}" method="POST" class="inline">
                                    ...
                                    <button
                                        type="submit"
                                        class="px-3 py-1 text-sm text-white bg-red-800 rounded hover:bg-red-900 border border-black font-medium"
        ...
        <form id="approvalForm" method="POST" enctype="multipart/form-data">
            ...
            <div class="mt-4 space-y-4">
                ...
                <div>
                    ...
                    <input
                        ...
                        name="approver_name"
                        required
                        class="w-full px-3 py-2 bg-input border border-black text-black rounded-md shadow-sm focus:ring-white focus:border-black"
                ...
                <div>
                    ...
                    @if(empty($user->e_signed))
                        <input
                            ...
                            class="w-full px-3 py-2 bg-input border border-black text-black rounded-md shadow-sm focus:ring-white focus:border-black"
                        >
                        <p class="mt-1 text-sm text-white">Upload your signature image (PNG, JPG, etc.)</p>
                    @else
                        <div class="flex items-center gap-4">
                            <img src="{{ asset('storage/' . $user->e_signed) }}" alt="E-Signature" class="h-16 border rounded bg-white p-1">
                            <span class="text-white text-sm">Signature on file</span>
                        </div>
                    @endif
                </div>
            ...
            <div class="mt-6 flex justify-end space-x-3">
                <button
                    ...
                    class="px-4 py-2 bg-secondary border border-white rounded text-white hover:bg-secondary/80 text-white text-sm"
                >
                    Cancel
                </button>
                <button
                    type="submit"
                    class="px-4 py-2 bg-primary border border-white rounded text-white hover:bg-primary/80 text-white text-sm"
                >
                    Approve Budget
                </button>
            </div>
